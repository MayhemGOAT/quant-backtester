...
from sentiment_analyzer import add_sentiment_features
...
SENTIMENT_COLS = [
    ...
    "Sentiment_Volatility_10",
    "Sentiment_Bullish",
    "Sentiment_Bearish",
]


def fetch_stock_data(ticker="AAPL", start_date="2010-01-01", end_date=None):
    if end_date is None:
...
def build_model(df):
    ...
    X_train, y_return_train, y_actual_train, prices_train, dates_train = (
        ...
        y_return[:train_end],
        y_actual[:train_end],
        current_prices[:train_end],
        dates[:train_end],
    )
    X_val, y_return_val, y_actual_val, prices_val, dates_val = (
        X[train_end:val_end],
        y_return[train_end:val_end],
        y_actual[train_end:val_end],
        current_prices[train_end:val_end],
...
def visualize_sentiment(df):
    print("\nGenerating FinBERT sentiment plot...")
    recent = df.tail(500)

    fig, axes = plt.subplots(2, 1, figsize=(15, 8), sharex=True)

    ax1 = axes[0]
    ax1.plot(recent.index, recent["Close"], color="blue", linewidth=2, label="Close")
    ax1.set_title("Price vs FinBERT Sentiment", fontsize=14, fontweight="bold")
    ax1.set_ylabel("Price ($)")
    ...
    ax2.plot(
        recent.index,
        recent["Sentiment_MA_5"],
        color="#A23B72",
        linewidth=1.5,
        alpha=0.8,
        label="5-Day Sentiment MA",
    )
    ax2.axhline(0, color="black", linestyle="-", linewidth=0.5, alpha=0.5)
    sentiment = recent["Sentiment_Score"].astype(float)
    ax2.fill_between(
        recent.index,
        sentiment,
        0,
    ...
    print("Saved: images/sentiment_analysis.png")
    plt.close()
...
def visualize_feature_importance(model, feature_columns):
    ...
    colors = ["#2E86AB" if f in SENTIMENT_COLS else "#6A4C93" for f in importance_df["Feature"]]
    plt.barh(importance_df["Feature"], importance_df["Importance"], color=colors)
    plt.xlabel("Importance")
    plt.title("Top 20 Feature Importances (blue = FinBERT sentiment)", fontsize=14, fontweight="bold")
...
def predict_next_day(model, df, feature_columns):
    ...
    print(f"   FinBERT:   {sentiment:+.3f} ({sentiment_label})")
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("FINPULSE — STOCK RETURN FORECASTING WITH FINBERT SENTIMENT")
    print("=" * 60)

    ticker = "AAPL"
    df = fetch_stock_data(ticker)
    df = add_all_features(df)
    df, _ = add_sentiment_features(df, ticker)
